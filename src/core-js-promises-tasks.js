...
function getPromise(number) {
  return new Promise((fulfield, rejected) => {
    if (number >= 0) {
      fulfield('promise that will be fulfilled');
    } else {
      rejected(new Error('promise that will be rejected'));
    }
  });
}
 ...
 */
function getFirstResolvedPromiseResult(romises) {
  const promise = Promise.any(romises);
...
function queuePromises(promises) {
  ...
  for (let index = 0; index < promises.length; index += 1) {
    promise = promise.then((elem) =>
      promises[index].then((el) => elem + String(el))
    );
  }
